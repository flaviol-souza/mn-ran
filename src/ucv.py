...
def mark_dscp_for_c2(host, dport=14550, dscp_class='EF'):
    """
    ...
    host.cmd(f"iptables -t mangle -A OUTPUT -p udp --dport {dport} -j DSCP --set-dscp-class {dscp_class}")
    # (Opcional) marca entradas destinadas ao host (ex.: GCS recebendo MAVLink)
    host.cmd(f"iptables -t mangle -A PREROUTING -p udp --dport {dport} -j DSCP --set-dscp-class {dscp_class}")
...
def main():
    ...
    parser.add_argument('--policy', default='policy.yaml', help='arquivo YAML com politicas de rede')
    ...
    policy_file = args.policy #os.environ.get("UCV_POLICY", "../policies/policy.yaml")
    policy = {}
    ...
    makeTerm(uav, term='gnome-terminal') 
    ...
    mark_dscp_for_c2(uav, dport=14550, dscp_class='EF')
    mark_dscp_for_c2(root, dport=14550, dscp_class='EF')
